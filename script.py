...
def check_language_support(deepl_api_key):
    ...
    if response.status_code == 200:
        ...
        messagebox.showerror(
            "API Error", "Error checking supported languages with DeepL API."
        )
        return []


def translate_text(
    deepl_api_key, google_api_key, text, target_lang, template_lang_code
):
    if len(text.split()) < 4:
        return google_translate_text(
            google_api_key, text, target_lang, template_lang_code
        )
    else:
...
def translate_and_populate(
    template_content, target_lang, deepl_api_key, google_api_key, template_lang_code
):
    translated_content = {}
    for key, value in template_content.items():
        translation = translate_text(
            deepl_api_key, google_api_key, value, target_lang, template_lang_code
        )
...
def get_google_supported_languages(google_api_key):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            messagebox.showerror(
                "API Error",
                f"Error fetching supported languages from Google Translate: HTTP {response.status_code}",
            )
            print(
        ...
        messagebox.showerror(
            "Network Error", f"Network error when fetching languages: {e}"
        )
        return []
...
def fallback_translation(
    ...
    google_translate_languages = [
        lang for lang in not_translated_languages if lang in supported_languages
    ]
    if not google_translate_languages:
        messagebox.showinfo(
            "No Translation Needed", "No additional languages require Google Translate."
        )
        return
    total_chars = count_characters(template_content) * len(google_translate_languages)
    if messagebox.askyesno(
        "Confirm Google Translation",
        f"You are about to translate {total_chars} characters for the following language codes with Google Translate: {', '.join(google_translate_languages)}. Do you want to proceed?",
    ):
        for lang in google_translate_languages:
            translated_content = {}
            for key, value in template_content.items():
                translation = google_translate_text(
                    google_api_key, value, lang, template_lang_code
                )
                if translation is not None:
                    translated_content[key] = translation
            if translated_content:
                with open(
                    os.path.join(i18n_folder_path, f"{lang}.json"),
                    "w",
                    encoding="utf-8",
                ) as f:
...
def deepL_cleanup_mode():
    ...
    google_api_key = simpledialog.askstring(
        "Google API Key", "Enter Google Translate API Key:"
    )
    if not google_api_key:
        messagebox.showinfo(
            "Cancelled",
            "DeepL Cleanup Mode cancelled (no Google Translate API key provided).",
        )
        return
    ...
    if template_content is None:
        return
    short_content_keys = [
        key for key, value in template_content.items() if len(value.split()) < 4
    ]
    deepl_api_key = simpledialog.askstring("DeepL API Key", "Enter DeepL API Key:")
    if not deepl_api_key:
        messagebox.showinfo(
            "Cancelled", "DeepL Cleanup Mode cancelled (no DeepL API key provided)."
        )
        return
            ...
            if lang_content:
                for key in short_content_keys:
                    if key in lang_content:
                        lang_content[key] = google_translate_text(
                            google_api_key,
...
def translate_from_key_mode():
    ...
    if not google_api_key or not deepl_api_key:
        messagebox.showinfo(
            "Cancelled", "Translation cancelled (API key not provided)."
        )
        return
    ...
    start_key = simpledialog.askstring(
        "Start Key", "Enter the key to start translation from:"
    )
    ...
    for lang_file in os.listdir(i18n_folder_path):
        ...
        if (
            lang_file.endswith(".json")
            and lang_code != template_lang_code
            and (
                lang_code.lower() in google_supported_languages
                or lang_code.upper() in deepl_supported_languages
            )
        ):
            ...
            for key in list(template_content.keys())[start_index:]:
                ...
                if (
                    len(text.split()) < 4
                    or lang_code.upper() not in deepl_supported_languages
                ):
                    translated_text = google_translate_text(
                        google_api_key, text, lang_code, template_lang_code
                    )
                else:
    ...
    messagebox.showinfo(
        ...
        "The translation from the specified key is complete for all language files.",
    )
...
def confirm_translation():
    if special_logic_var.get() == 1:
        ...
        template_file_path = os.path.join(
            i18n_folder_path, f"{template_lang_code}.json"
        )
        if not os.path.isfile(template_file_path):
            messagebox.showerror(
                "Error",
                f"No template file found for the language code '{template_lang_code}'.",
            )
            return
        template_content = read_json(template_file_path)
        if template_content is None:
        ...
        if not deepl_api_key:
            messagebox.showinfo(
                "Cancelled", "Translation cancelled (no DeepL API key provided)."
            )
            return
        google_api_key = simpledialog.askstring(
            "Google API Key", "Enter Google Translate API Key:"
        ...
        if not google_api_key:
            messagebox.showinfo(
                "Cancelled",
                "Translation cancelled (no Google Translate API key provided).",
            )
            return
        target_languages = [
            file.split(".")[0]
            for file in os.listdir(i18n_folder_path)
            if file.endswith(".json") and file != f"{template_lang_code}.json"
        ]
        not_translated_languages = []
        ...
        if messagebox.askyesno(
            "Confirm Translation",
            f"You are about to translate {total_chars} characters for the following language codes: {', '.join(target_languages)}. Do you want to proceed?",
        ):
            start_translation(
                i18n_folder_path,
                template_lang_code,
...
def start_translation(
    ...
    for target_lang_code in target_languages:
        translated_content = translate_and_populate(
            template_content,
            target_lang_code,
            deepl_api_key,
            google_api_key,
        ...
        if translated_content:
            with open(
                os.path.join(i18n_folder_path, f"{target_lang_code}.json"),
                "w",
                encoding="utf-8",
            ) as f:
                json.dump(translated_content, f, ensure_ascii=False, indent=4)
    ...
    if not_translated_languages:
        messagebox.showinfo(
            "Partial Completion",
            "Initial translation is complete. Enter your Google Translate API key for additional languages.",
        )
        google_api_key = simpledialog.askstring(
            "Google API Key", "Enter Google Translate API Key:"
        )
...
Checkbutton(
    root, text="Translate from Specific Key", variable=start_from_key_var
).pack()
...
Button(
    root, text="Calculate Characters and Translate", command=confirm_translation
).pack()
